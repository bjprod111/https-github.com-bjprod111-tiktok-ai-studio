...
app.get('/api/health', (_req, res) => {
  ...
  res.json({ ok: true, service: 'new-age-news-backend', sources: row.count, articles: articles.count });
});

app.get('/api/sources', (_req, res) => {
  res.json(db.prepare('SELECT * FROM sources ORDER BY name').all());
});
...
app.post('/api/ingest', async (_req, res) => {
  res.json({ results: await ingestEnabledSources() });
});
...
app.get('/api/feed', (req, res) => {
  ...
  const articles = db.prepare(`
    SELECT id, source_id AS sourceId, source_name AS sourceName, source_url AS sourceUrl,
           article_url AS articleUrl, title, description, author, published_at AS publishedAt,
           fetched_at AS fetchedAt, image_url AS imageUrl, language, country, category,
           summary, relevance_score AS relevanceScore, freshness_score AS freshnessScore,
           importance_score AS importanceScore, story_cluster_id AS storyClusterId
    FROM articles ORDER BY COALESCE(published_at, fetched_at) DESC LIMIT ?
  `).all(limit);
  res.json({ articles });
});

app.listen(port, () => console.log(`NEW AGE NEWS backend running at http://localhost:${port}`));
